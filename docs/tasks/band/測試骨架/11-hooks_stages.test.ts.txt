...
import { CONTENT_ROWS_MAX, layout, nextStage, rowsForStage, STAGE_ORDER, type Stage } from "../../plugins/telltale/hooks/layout";
...
test("clicking a title and running `/telltale agents size full` both write store key size.agents", async () => {
  ...
  // path 1: two title clicks cycle summary -> compact -> full via ui.message
...
test("ui.render wants: a staged panel's minRows/wantRows follow size.<id>, not its static Panel fields", async () => {
  ...
  // layout() has the whole content budget to give it, so its rendered row count must be
  // CONTENT_ROWS_MAX (7), not its static Panel.wantRows (3).
  const client = clientOf(tree);
  const props = client!.props as { panels: { id: string; rows: number }[] };
  expect(props.panels.find((p) => p.id === "agents")?.rows).toBe(CONTENT_ROWS_MAX);
});
