...
"""突變：對「這個行為很重要」的每一行，故意改壞、跑 bun test、要求它轉紅、改回來。
    ...
    make mutate            # 讀 tests/突變/*.json，全部跑
    uv run python scripts/突變.py --只 layout   # 只跑 label 含 "layout" 的

清單格式（tests/突變/NN-*.json，每張票一檔；file 相對 repo 根）：
    [{"label": "砍掉的面板不講出來", "file": "plugins/telltale/hooks/layout.ts",
      "old": "dropped.push(id)", "new": "void id"}]

規矩（題目 §5.2）：
- `old` 在檔案裡必須恰好出現一次；0 次或多次都是清單壞了，退出碼 2，不猜。
- 突變後 `bun test <測試目錄>` 必須非 0；全綠 = 那條承諾沒有測試守，退出碼 1，印出哪幾條。
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--清單", default="tests/突變")
