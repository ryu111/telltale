...
CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1 claude --plugin-dir "$PWD" --debug-file /tmp/tt.log   # 開發模式，存檔熱重載
claude plugin validate --strict .      # calls: 那行只准 $.ui.* $.clock.* $.store.* $.command.register $.env.get
```
