...
declare module 'claude-code' {
  interface BuiltinToolInputs {
    ...
    Artifact: {
      ...
      data?: {}
      /** Path to the .html file to render — or, for an Artifact created from an Artifact type, one of its own data files. Required to publish (the default action), except with `type_url`. Use a short, distinctive basename — it is the last-resort title when the HTML has no <title> and no `title` parameter is given. For 'upload_asset', the local image, video, PDF, font, or text (CSV, Markdown, JSON, plain text) file to upload. For 'write_db' (db_op 'set' or 'update'), a local JSON file whose top-level object is sent as the document — an alternative to inline `data`, so a large document need not pass through the conversation. */
      file_path?: string
      /** Browser-tab icon: one or two emoji (e.g. "📊"). No markup. Required on a page's first publish; omit on a redeploy (same file path this session, or `url`) to keep the artifact's icon — pass a new one only when the user asks. Optional for data files on an Artifact created from an Artifact type and with `type_url` (the type's icon stays unless you pass one with files). */
      favicon?: string
      /** Supporting files to publish alongside the page. Map form {"published/path": "source/path" | {from, contentType} | {artifact, path, ver?} | null} publishes each source at the key (what the HTML references) — an {artifact, path} source copies that Artifact's published file server side (an Artifact you can open, same organization; its type comes with it; not an HTML, SVG or XML document; at most 4 source Artifact versions per publish); when updating an existing artifact, files left out of the map are kept and null removes that path. List form publishes each file at its own spelling. Local sources must lie under the working directory or your scratchpad directory (as your system prompt names it). `preflight.js` at the artifact root is reserved — it runs against open pages when you publish updates; it must be a ≤ 8 KiB ES module whose default export is a function, or the publish is refused. */
      files?: Array<{
      ...
      out_dir?: string
      /** read_file only: the file's published path inside the artifact, exactly as list_files printed it; "index.html" is the page itself. */
      path?: string
      ...
      from_url?: string
      /** copy_from only: 1–10 distinct asset ids from the source artifact (its list_assets or upload_asset results). */
      asset_ids?: string[]
      /** read only: pass page: true for the rendered page itself where a read otherwise answers something else — a read of an Artifact created from an Artifact type leaves its page out when the type's instructions come with it (the page is the type's own, the same on every Artifact made from it). */
      page?: boolean
