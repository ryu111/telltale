// Ticket 19: same assertions as tests/hooks/width.test.ts, rewritten against
// claude-code/testing to evaluate whether `claude plugin test` can replace
// the bun harness. SDD §0.1, §4.
...
test("astral-plane CJK (surrogate pairs) and the compatibility ranges are wide", () => {
  ...
  expect(displayWidth("豈")).toBe(2); // CJK Compatibility Ideographs
