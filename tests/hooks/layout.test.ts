...
// Ticket 23: the status row is conditional; the arithmetic below passes `{ status: true }`
// where it assumed FIXED_ROWS, and the rule-5 checks use (status ? FIXED : TITLE).
import { expect, test } from "bun:test";
import { BAND_ROWS_MAX, CONTENT_ROWS_MAX, FIXED_ROWS, TITLE_ROWS, MIN_COLUMNS, PANEL_TITLE_ROWS, layout } from "../../plugins/telltale/hooks/layout";

const P = (id: string, minRows: number, wantRows: number) => ({ id, minRows, wantRows });
const WITH_STATUS = { status: true };
...
test("rule 3: a panel that cannot get minRows is dropped whole, never half", () => {
  ...
  const r = layout([P("a", 3, 3), P("b", 3, 3)], FIXED_ROWS + 4, WITH_STATUS);
...
test("rule 3: a later, smaller panel still fits after an earlier drop", () => {
  const r = layout([P("big", 5, 5), P("small", 1, 1)], FIXED_ROWS + 2, WITH_STATUS);
...
test("rule 1: budget is computed once, not re-judged per panel", () => {
  ...
  const r = layout([P("a", 2, 2), P("b", 1, 1)], FIXED_ROWS + 3, WITH_STATUS);
...
test("rule 4: leftover rows top up to wantRows in order; slack is allowed", () => {
  ...
  const r = layout([P("a", 1, 2), P("b", 1, 3)], FIXED_ROWS + 6, WITH_STATUS);
  ...
  expect(slack.total).toBe(TITLE_ROWS + 1 + 1);
});
...
test("no panels wanted: one row, nothing dropped, no status row", () => {
...
test("rule 5: every drawn panel costs its title row: total = (status ? FIXED : TITLE) + Σ(1 + rows)", () => {
  for (const maxRows of [3, 4, 5, 7, 9, 40]) {
    const r = layout([P("a", 1, 2), P("b", 1, 1)], maxRows);
    const fixed = r.status ? FIXED_ROWS : TITLE_ROWS;
    expect(r.total).toBe(fixed + r.slots.reduce((n, s) => n + PANEL_TITLE_ROWS + s.rows, 0));
  }
  // maxRows 3 → status shown once a is dropped → budget 1: not even one panel (title + 1) fits
  expect(layout([P("a", 1, 1)], 2)).toEqual({ slots: [], dropped: ["a"], total: FIXED_ROWS, status: true });
});
